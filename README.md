...
- `export-workspace/playwright-export-gemini/` と `export-workspace/workdir/` を**兄弟**にし、常に前者だけに `cd` して実行すると、モノレポへ向けるパスが `../workdir/...` になります。設定では **`..` を含む相対パスは使えない**ため、この配置のままでは困りがちです。**親を1つにまとめる**（この例Cの形）か、実行時は必ず `workdir` を子として含む親に `cd` してください。
...
6. ルートにいる状態で `npm run export:gemini:check` を試し、問題なければ `npm run export:gemini` を実行する。
...
2. 対象プロジェクトの `package.json` に、依存として `yaml`（例: `"yaml": "^2.8.3"`）と、`export:gemini` / `export:gemini:check`（Pack利用時は`export:gemini:pack`）用の `scripts` を追加する（中身はこのリポジトリの `package.json` をそのまま流用できる）。
...
5. **必ず対象プロジェクトのルートに `cd` した状態**で `npm run export:gemini:check` → 問題なければ `npm run export:gemini` を実行する。
...
```bash
npm run export:gemini:pack
```

（同等: `npm run export:gemini -- --pack`）

`--check`と併用した場合は`_pack/`は作らず、標準出力にchunk/bundle数の概算のみ出します。

```bash
npm run export:gemini:check -- --pack
```

`.gemini-export.json` の `pack`（`chunkMode` / `chunkMaxLines` / `maxChunkBytes` / `bundleGroupDepth` / `outSubDir`）で調整できます。既定値は [tools/gemini-export/default-config.mjs](tools/gemini-export/default-config.mjs) の `pack` を参照してください。

## CI での推奨

- **本番相当のリポジトリ**では、パイプラインで`npm run export:gemini`（または先に`export:gemini:check`）を実行し、**`failOnWarnings: true`**を有効にすると、機密っぽい検知・設定ミス・セキュリティ関連スキップを警告のまま放置しにくくなります。設定例の断片は[templates/gemini-export.ci.snippet.json](templates/gemini-export.ci.snippet.json)を参照し、`.gemini-export.json`にマージしてください。
...
.ai-context/playwright-gemini/
  ...
  _pack/             # `npm run export:gemini -- --pack` 実行時のみ
```

サニタイズ済みツリーに加え、Packを付けた場合は`_pack/`内の`PROJECT_INDEX.md`や`bundles/`をNotebookLMやGeminiチャットに渡す運用が可能です。
