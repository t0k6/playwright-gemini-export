...
- GeminiとNotebookLMを併用する運用方針は [`docs/gemini-context-strategy.md`](docs/gemini-context-strategy.md) を参照
- `indexChunk`（`PROJECT_INDEX.md` / `PATH_INDEX.jsonl` / `chunks/`）の仕様は [`docs/gemini-index-chunk.md`](docs/gemini-index-chunk.md) を参照
- 今後の機能拡張や優先度は [`docs/gemini-export-roadmap.md`](docs/gemini-export-roadmap.md) を参照
- このREADMEはセットアップと基本運用を中心にし、`bundle` 戦略の詳細は別文書に分離する
...
export 後に `outDir` へ `PROJECT_INDEX.md`・`PATH_INDEX.jsonl`・`chunks/` を追加する。`.gemini-export.json` で有効化する。
...
既定の `projectIndexFile` / `pathIndexFile` / `chunksDir` は [tools/gemini-export/default-config.mjs](tools/gemini-export/default-config.mjs) の `indexChunk` を参照。

設定を変えずに一度だけ有効化する場合は CLI に `--index-chunk` を付ける。

```bash
npm run export:gemini:pw -- --index-chunk
```

dry-run で件数だけ確認する場合。

```bash
npm run export:gemini:pw:check -- --index-chunk
```
...
より大きなPlaywrightプロジェクトをGeminiやNotebookLMへ渡すときの情報設計は [`docs/gemini-context-strategy.md`](docs/gemini-context-strategy.md) にまとめている。
