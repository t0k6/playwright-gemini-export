...
- `summary`…将来のrich metadata用。現状は空文字でプレースホルダー（[roadmap.md](roadmap.md)参照）
...
`bundle`、PATH_INDEXの`summary`充填、chunkの意味単位分割、`index-chunk`モジュールの分割などは[roadmap.md](roadmap.md)の「ロードマップ」および「今後の課題（計画で見送り・低優先）」節に整理する。
